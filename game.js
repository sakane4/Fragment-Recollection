...
function _addTotalFragments(amount) {
  ...
  const lvCap = state.allCompanionsMetDone ? WORLD_LV_THRESHOLDS.length : 10;
  while (newLv < lvCap && newTotal >= WORLD_LV_THRESHOLDS[newLv]) {
    newLv++;
  }
...
// シナリオから依頼を直接開始する。すでに開始済みなら何もしない。
function activateQuest(questId) {
  const quest = getQuestDefinition(questId);
  if (!quest || getQuestStatus(state, questId) !== QUEST_STATUS.UNAVAILABLE) return { ok: false };
  const actionBaselines = Object.fromEntries(
    (quest.baselineActions ?? []).map(actionId => [actionId, state.actionCount?.[actionId] ?? 0])
  );
  state = {
...
function completeAction(actionId, onComplete) {
  ...
  const lostFlowersLibraryBaseline =
    ...
    ?? state.actionCount?.nostalgia_library_research
    ?? 0;
  const lostFlowersLibraryCount =
    (state.actionCount?.nostalgia_library_research ?? 0) - lostFlowersLibraryBaseline;
  if (
...
function init() {
  ...
  state = {
    ...
    knownConstellationEffectIds: saved.knownConstellationEffectIds ?? INITIAL_STATE.knownConstellationEffectIds,
  ...
  if (
    getQuestStatus(state, 'lost_flowers') === QUEST_STATUS.ACTIVE &&
    !state.questActionBaselines?.lost_flowers
  ) {
    state = {
      ...state,
      questActionBaselines: {
        ...state.questActionBaselines,
        lost_flowers: {
          nostalgia_library_research: state.actionCount?.nostalgia_library_research ?? 0,
        },
      },
    };
    saveToStorage(state);
...
function completeTericiaConstellation(constellation) {
  ...
  const knownConstellationEffectIds = state.knownConstellationEffectIds?.includes(effect.id)
    ? state.knownConstellationEffectIds
    : [...(state.knownConstellationEffectIds ?? []), effect.id];
  state={
    ...state,
    tericiaConstellationEditing:false,
...
function deleteCustomConstellation(id) {
  const before = state.customConstellations ?? [];
  const next = before.filter(item => item.id !== id);
  if (next.length === before.length) return { ok:false };
  state = { ...state, customConstellations: next };
  saveToStorage(state);notify();
  return { ok:true };
}
